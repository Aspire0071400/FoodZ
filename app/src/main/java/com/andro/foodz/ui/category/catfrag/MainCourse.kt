...
class MainCourse : Fragment() {
    lateinit var fragmentMaincourse : FragmentMaincourseBinding
    private var data = arrayListOf<CatNavDataModel>()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?
    ): View {
        fragmentMaincourse = FragmentMaincourseBinding.inflate(inflater)
        fetchData()
        return fragmentMaincourse.root
    }
    ...
    private fun fetchData() {
        ...
        val stringRequest = StringRequest(
            Request.Method.GET,"https://foodzproject.000webhostapp.com/fetch_data.php" , {
                ...
                fragmentMaincourse.cat1NavDataViewcycle.layoutManager = LinearLayoutManager(context)
                fragmentMaincourse.cat1NavDataViewcycle.adapter = CatNavDataAdapter(data)
